...
# ── GPU settings ──────────────────────────────────────────
torch.backends.cudnn.deterministic = True   # eliminates GPU rounding differences
torch.backends.cudnn.benchmark     = False  # keeps results identical across runs
# ──────────────────────────────────────────────────────────
...
def evaluate(model, X_te, y_te, threshold=0.95):
    ...
    # ── Find optimal threshold by best F1 ─────────────────
    prec_arr, rec_arr, thresholds = precision_recall_curve(labels, probs)
    f1_scores = 2 * prec_arr * rec_arr / (prec_arr + rec_arr + 1e-8)
    best_thresh = thresholds[f1_scores[:-1].argmax()]
    print(f"\nBest threshold by F1: {best_thresh:.4f}  (you passed: {threshold})")
    print("Re-evaluating with best threshold...")
